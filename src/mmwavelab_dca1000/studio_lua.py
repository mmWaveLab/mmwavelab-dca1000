...
def generate_iwr1843_studio_lua(config: StudioLuaConfig | None = None) -> str:
    ...
    progress_path = _lua_string(cfg.progress_path)
    metrics = cfg.metrics()
...
function mark(label, ret)
    if PROGRESS_PATH == "" then
        return
    end
    local f = io.open(PROGRESS_PATH, "a")
    if f ~= nil then
        f:write(os.date("%Y-%m-%d %H:%M:%S"), ",", tostring(label), ",", tostring(ret), "\\n")
        f:close()
    end
end
...
function mmwl_call(label, fn)
    ...
    if ok == false then
        ...
        mark(label, "exception:" .. tostring(ret))
        return -9999
    end
    if ret == 0 then
...
mark("script_start", "COM" .. tostring(COM_PORT))
...
if connect_ret ~= 0 then
    ...
    mark("abort_connect", connect_ret)
    return
end
...
if connected_ret ~= 0 then
    ...
    mark("abort_is_connected", connected_ret)
    return
end
...
mark("script_done", SAVE_DATA_PATH)
"""
