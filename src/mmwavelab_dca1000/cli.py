...
from .dca1000_py import Dca1000Py, summarize_results
...
def cmd_dca_py_status(args: argparse.Namespace) -> int:
    config = DCA1000Config.load(args.dca_json)
    dca = Dca1000Py.from_config(config, timeout_s=args.timeout_s)
    results = [dca.system_connect(), dca.reset_fpga(), dca.fpga_version(), dca.config_fpga(), dca.config_packet_data(packet_delay_us=args.packet_delay_us)]
    report = {
        "host_ip": dca.host_ip,
        "dca_ip": dca.dca_ip,
        "config_port": dca.config_port,
        "data_port": dca.data_port,
        "results": summarize_results(results),
    }
    print(json.dumps(report, ensure_ascii=False, indent=2))
    return 0 if all(item.ok for item in results if item.command != "fpga_version") else 1


def cmd_capture_sdk_demo_py(args: argparse.Namespace) -> int:
    out = Path(args.output_dir or Path("captures") / f"sdk_demo_py_capture_{time.strftime('%Y%m%d_%H%M%S')}")
    data_dir = out / "data"
    meta_dir = out / "meta"
    data_dir.mkdir(parents=True, exist_ok=True)
    meta_dir.mkdir(parents=True, exist_ok=True)
    ...
    dca_config = DCA1000Config.load(args.dca_json)
    dca_config.set_capture_path(file_base_path=data_dir.resolve(), file_prefix=args.file_prefix, duration_ms=args.duration_ms)
    runtime_json = dca_config.save(meta_dir / "dca1000_runtime_config.json")
    dca = Dca1000Py.from_config(dca_config, timeout_s=args.dca_timeout_s)
    dca_results = dca.configure_from_json(dca_config)
    capture_stats = None
    start_record = None
    stop_record = None
    sensor_start = None
    ...
    if config_result.ok and all(result.ok for result in dca_results if result.command != "fpga_version"):
        raw_udp_path = data_dir / f"{args.file_prefix}_raw_udp.bin"
        adc_payload_path = data_dir / f"{args.file_prefix}_adc_payload.bin"
        holder: dict[str, object] = {}

        def _reader() -> None:
            holder["stats"] = dca.capture_udp(
                raw_udp_path=raw_udp_path,
                adc_payload_path=adc_payload_path,
                duration_s=max(args.duration_ms / 1000.0 + args.capture_grace_s, args.capture_grace_s),
                strip_sequence_header=not args.keep_udp_headers,
                stop_event=stop_event,
            )

        capture_thread = threading.Thread(target=_reader, name="dca1000-py-capture", daemon=True)
        capture_thread.start()
        time.sleep(args.record_arm_delay_s)
        start_record = dca.record_start()
        time.sleep(args.sensor_start_delay_s)
        sensor_start = send_mmwave_cli_commands(args.com_port, ["sensorStart"], baudrate=args.baudrate, timeout_s=args.command_timeout_s)
        time.sleep(args.duration_ms / 1000.0)
        if args.stop_sensor:
            sensor_stop = send_mmwave_cli_commands(args.com_port, ["sensorStop"], baudrate=args.baudrate, timeout_s=args.command_timeout_s)
        stop_record = dca.record_stop()
        stop_event.set()
        capture_thread.join(timeout=5.0)
        capture_stats = holder.get("stats")
    report = {
        "output_dir": str(out),
        "radar_cfg": str(args.radar_cfg),
        "dca_json": str(runtime_json),
        "com_port": args.com_port,
        "baudrate": args.baudrate,
        "pure_python": True,
        "config_result": asdict(config_result),
        "dca_results": summarize_results(dca_results),
        "start_record": start_record.as_dict() if start_record else None,
        "stop_record": stop_record.as_dict() if stop_record else None,
        "sensor_start": asdict(sensor_start) if sensor_start else None,
        "sensor_stop": asdict(sensor_stop) if sensor_stop else None,
        "capture_stats": capture_stats.as_dict() if hasattr(capture_stats, "as_dict") else None,
        "files": [
            {"path": str(path), "size_bytes": path.stat().st_size}
            for path in sorted(out.glob("**/*"))
            if path.is_file()
    ...
    (out / "sdk_demo_py_capture_report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps(report, ensure_ascii=False, indent=2))
    data_files = [item for item in report["files"] if str(item["path"]).lower().endswith(".bin") and item["size_bytes"] > 0]
    return 0 if data_files else 1
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("dca-py-status")
    p.add_argument("--dca-json", default="configs/dca1000_iwr1843.json")
    p.add_argument("--timeout-s", type=float, default=2.0)
    p.add_argument("--packet-delay-us", type=int, default=25)
    p.set_defaults(func=cmd_dca_py_status)

    p = sub.add_parser("capture-sdk-demo-py")
    p.add_argument("--com-port", required=True)
    p.add_argument("--baudrate", type=int, default=115200)
    p.add_argument("--radar-cfg", default="configs/iwr1843_best_range_1tx_256s_3983mhz.cfg")
    p.add_argument("--dca-json", default="configs/dca1000_iwr1843.json")
    ...
    p.add_argument("--command-timeout-s", type=float, default=1.5)
    p.add_argument("--inter-command-delay-s", type=float, default=0.05)
    p.add_argument("--record-arm-delay-s", type=float, default=0.2)
    p.add_argument("--sensor-start-delay-s", type=float, default=0.1)
    p.add_argument("--capture-grace-s", type=float, default=2.0)
    p.add_argument("--dca-timeout-s", type=float, default=2.0)
    p.add_argument("--keep-udp-headers", action="store_true")
    p.add_argument("--stop-sensor", action="store_true")
    p.set_defaults(func=cmd_capture_sdk_demo_py)
